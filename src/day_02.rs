...
pub fn part_2(test_data: TestData) -> String {
    let mut invalid_ids : Vec<(String, u8)> = Vec::new();
    ...
    while let Some(range) = words.next() {
        ...
        while str_num_compare(&id, high) != cmp::Ordering::Greater {
            ...
            for repeats in 2..=id.len() {
                ...
                if str_num_compare(&mirror_id, high) != cmp::Ordering::Greater && str_num_compare(&mirror_id, low) != cmp::Ordering::Less {
                    invalid_ids.push((id_frac, repeats as u8));
                    log::debug(|| format!("Low: {low}, High: {high}, Current: {mirror_id}"));
                    log::info(|| format!("Invalid ID Found {mirror_id}"));
                    if str_num_compare(&mirror_id, &id) == cmp::Ordering::Greater {
                        id = mirror_id;
                    }
                }
            }
            id = (id.parse::<usize>().unwrap() + 1).to_string();
        }
    }

    let mut sum : usize = 0;
    for (invalid, repeats) in invalid_ids {
        let rep_id = invalid.repeat(repeats as usize);
        let full_id = rep_id.parse::<usize>().unwrap();
        sum += full_id;
    }
